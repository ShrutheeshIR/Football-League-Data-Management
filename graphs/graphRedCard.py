...
def graphredcard(tid=None):
    ...
    if tid==None:
        mycursor.execute("select count(eid),matches.season from event,matches where event.`Type`='R' and event.mid=matches.mid group by matches.season ;")
    else:
